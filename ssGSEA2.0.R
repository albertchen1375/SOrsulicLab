...
# Helper function to extract argument values from flags (e.g., --input.ds=file.gct)
get_arg <- function(flag) {
  flag_idx <- grep(flag, args)
  if (length(flag_idx) == 0) return(NA)
  gsub(paste0(flag, "="), "", args[flag_idx])
}

# Parse arguments
input.ds           <- get_arg("--input.ds")
output.prefix      <- get_arg("--output.prefix")
gene.set.databases <- get_arg("--gene.set.databases")
sample.norm.type   <- get_arg("--sample.norm.type")
weight             <- as.numeric(get_arg("--weight"))
statistic          <- get_arg("--statistic")
output.score.type  <- get_arg("--output.score.type")
nperm              <- as.numeric(get_arg("--nperm"))
combine.mode       <- get_arg("--combine.mode")
correl.type        <- get_arg("--correl.type")
min.overlap        <- as.numeric(get_arg("--min.overlap"))
par                <- as.logical(get_arg("--par"))
spare.cores        <- as.numeric(get_arg("--spare.cores"))
...
script.dir <- getwd()  # Sets script.dir to the current working directory
...
# Call ssGSEA2 with command-line arguments
ssGSEA2(
  input.ds = input.ds,
